...
-   \-rwx------：普通文件，所属用户可读、写和执行该文件。其它用户没有任何权限。
-   \-rw-------：普通文件，所属用户可读和写。其它用户没有任何权限。
-   \-rw-r--r--：普通文件，所属用户可读和写，所属用户组可读，所有用户可读。
-   \-rwx-rx-x：普通文件，所属用户可读、写和执行，所属用户组可读和执行，其它所有用户可执行。
-   \-rw-rw----：普通文件，文件所属者和所属用户组可读和写，其它用户没有任何权限。
-   lrwxrwxrwx：软链接，后面的9个属性字符无含义，其权限取决于指向的文件权限。
-   drwxrwx---：文件夹，文件所属者和用户组可进入文件夹，在内创建、重命名和移动文件。
-   drwxr-x---：文件夹，所属用户可进入文件夹，在内创建、重命名和移动文件夹；所属用户组可进入文件夹，但不可在内创建、重命名和移动文件夹。
...
_其中常用的有7（rwx）、6（rw-）、5（r-x）、4（r--）和0（---）这几种模式。_
...
drwxr-xr-x 2 root root 4096 8月  21 16:47 /usr/local/share/Music
```
...
-rw-rw-r-- 1 bill music 0 8月  22 06:16 /usr/local/share/Music/test_file2
bill@ubuntu16.04:~$ mkdir /usr/local/share/Music/test_dir2
bill@ubuntu16.04:~$ ls -ld /usr/local/share/Music/test_dir2
drwxrwsr-x 2 bill music 4096 8月  22 06:17 /usr/local/share/Music/test_dir2
```
